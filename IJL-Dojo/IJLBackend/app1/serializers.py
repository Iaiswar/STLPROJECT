...
class IntakeEntrySerializer(serializers.ModelSerializer):
    department_name = serializers.CharField(source='department.name', read_only=True)
    temp_id = serializers.CharField(read_only=True)  # Add temp_id

    class Meta:
        model = IntakeEntry
        fields = [
            'id', 'temp_id', 'sr_no', 'contractor_name', 'category',
            'candidate_name', 'education', 'bio_data_submitted',
            'department', 'department_name', 'remark', 'status', 
            'created_at', 'updated_at' 
        ]
        read_only_fields = ['temp_id', 'created_at', 'updated_at']
    ...
    def create(self, validated_data):
        with transaction.atomic():
            ...
            for index, entry_data in enumerate(entries_data):
                ...
                # Note: temp_id and status will be auto-generated by the model
...
# Add this new serializer for displaying individual intake entries
class IntakeEntryDisplaySerializer(serializers.ModelSerializer):
    department_name = serializers.CharField(source='department.name', read_only=True)
    # sheet_ref_no = serializers.CharField(source='sheet.ref_no', read_only=True)
    sheet_date = serializers.CharField(source='sheet.date_created', read_only=True)
    
    class Meta:
        model = IntakeEntry
        fields = [
            'id', 'temp_id', 'candidate_name', 'contractor_name', 'category',
            'education', 'department_name', 'bio_data_submitted', 'remark',
            'status', 'created_at', 'updated_at', 'sheet_date'
            #   'sheet_ref_no',
        ]
